...
describe "wonderstuff::default" do
  ...
  let(:chef_run) { ChefSpec::SoloRunner.new.converge('recipe[wonderstuff::default]') }  
  ...
  it "creates a webpage to be served" do
    expect(chef_run).to render_file('/var/www/index.html').with_content('Wonderstuff Design')
  end
